...
void FetchPackage::StartFetchURL(string _url, string _fileName) {
	try {
		cout << "Fetching package from " << _url << "\n";
		cout << "Preparing download...\n";
		//cin.get();
...
string FetchPackage::GetTempDirectory() {
	...
	char path = getenv("TMPDIR"); //get the temp enviornment
	if (path) return string(path);//condensed if statement
	return "/tmp/";
